...
package object pinpoint {
  ...
  def readSettingsFromProjectFile(filename: String = "pinpoint-cfg.json") = {
    val jsonString = FileUtils.readFileToString(new File(filename), "utf8")
  ...
  private[pinpoint] def reset() = timelines.clear()
  ...
  def log(msg: String, level: Int = 0, readSettings: => Settings = readSettingsFromProjectFile()): Unit = {
    ...
    while (timelines.length < settings.markers.length + 1)
      timelines.append(ListBuffer.empty[String])
    ...
    if (level < timelines.length && allowedLogAtLevel) {  // Ignore levels beyond the innermost timeline
